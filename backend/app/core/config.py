...
class Settings(BaseSettings):
    ...
    # ────────────────────────────────────────────────────────────────────────
    """
    PATCH: app/core/config.py
    ADD THESE LINES TO THE Settings CLASS

    Find the Settings class definition and add these fields:
    """
   ...
   # JWT Configuration
    ...
    ADMIN_USERS: list[str] = Field(
        default_factory=list, # Best practice for mutable types like lists
        description="List of admin usernames"
    )
    # INSTRUCTIONS:
    # 1. Open your existing app/core/config.py
    # 2. Find the Settings class (usually starts with: class Settings(BaseSettings):)
    # 3. Copy all the fields above and paste them BEFORE the closing of the Settings class
    # 4. Make sure indentation matches the other fields
    # 5. You can now use settings.SECRET_KEY in your auth.py file
